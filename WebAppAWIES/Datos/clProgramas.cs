...
namespace WebAppAWIES.Datos
{
    public class clProgramas
    {
        ...
        {
            string consulta = " select Programas.NombrePrograma ,Universidades.Codigo, Universidades.NombreInstitucion, Departamento.Departamento, Municipio.Municipio, EstadoPrograma, NivelAcademico,ReconocimientoMinisterio, NivelFormacion.NivelFormacion,Metodologia.Metodologia, Area.Area, Precio, NumeroCreditos, VigenciaAños, DuracionSemestre,TituloOtorgado,Programas.TipoIngreso, Programas.PrimerIngresado, Programas.UltimoIngresado  from Programas INNER JOIN NivelFormacion ON (NivelFormacion.IdNivelFormacion=Programas.IdNivelFormacion) INNER JOIN Metodologia ON (Metodologia.IdMetodologia = Programas.IdMetodologia) INNER JOIN Area ON (Area.IdArea = Programas.IdAreaConocimiento) INNER JOIN InstitucionPrograma ON (Programas.IdProgramas = InstitucionPrograma.IdProgramas) INNER JOIN Universidades ON (Universidades.IdUniversidades = InstitucionPrograma.IdUniversidades) INNER JOIN Municipio On (Universidades.IdMunicipio = Municipio.IdMunicipio) INNER JOIN Departamento On (Departamento.IdDepartamento = Municipio.IdDepartamento) where Programas.IdProgramas = '" + id + "'";
            DataSet dsInstituto = new DataSet();
            dsInstituto = objConexion.mtdDesconectado(consulta);
            return dsInstituto;
        }
        ...
        {
            string query = "select Programas.IdProgramas, Programas.NombrePrograma, NombreInstitucion,  NivelFormacion, Metodologia, NivelAcademico from Universidades INNER JOIN InstitucionPrograma ON(Universidades.IdUniversidades = InstitucionPrograma.IdUniversidades) INNER JOIN Programas ON (Programas.IdProgramas = InstitucionPrograma.IdProgramas) INNER JOIN NivelFormacion ON (NivelFormacion.IdNivelFormacion = Programas.IdNivelFormacion) INNER JOIN Metodologia ON (Metodologia.IdMetodologia = Programas.IdMetodologia) WHERE NivelAcademico ='Posgrado'";
            DataSet tblPrograma = new DataSet();
            tblPrograma = objConexion.mtdDesconectado(query);
            return tblPrograma;
        }
        ...
        {
            string query = "select Programas.IdProgramas, Programas.NombrePrograma, NombreInstitucion,  NivelFormacion, Metodologia, Area.Area  from Universidades INNER JOIN InstitucionPrograma ON(Universidades.IdUniversidades = InstitucionPrograma.IdUniversidades) INNER JOIN Programas ON(Programas.IdProgramas = InstitucionPrograma.IdProgramas) INNER JOIN NivelFormacion ON(NivelFormacion.IdNivelFormacion = Programas.IdNivelFormacion) INNER JOIN Metodologia ON(Metodologia.IdMetodologia = Programas.IdMetodologia) INNER JOIN Area ON(Area.IdArea = Programas.IdAreaConocimiento) WHERE Area = 'AGRONOMIA, VETERINARIA Y AFINES'";
            DataSet tblPrograma = new DataSet();
            tblPrograma = objConexion.mtdDesconectado(query);
            return tblPrograma;
        }
